...
class EducationFeeController extends GetxController {
  ...
  Future<void> fetchBillCountries() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint: "${ApiPath.getBillCountriesEndpoint}/education-fee",
      );
  ...
  Future<void> fetchPayBillServices() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint:
            "${ApiPath.getPayBillServicesEndpoint}/${countryController.text}/education-fee",
      );
      if (response.status == Status.completed) {
  ...
  bool validateAmountStep() {
    ...
    if (countryController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.educationFeeCountryRequired);
      return false;
    }

    // Validate Service
    if (serviceController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.educationFeeServiceRequired);
      return false;
    }

    // Validate Amount
    if (amountText.value.isEmpty && amountController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.educationFeeAmountRequired);
      return false;
    }

    final amount = double.tryParse(amountController.text) ?? 0.0;
    if (amount <= 0) {
      ToastHelper().showErrorToast(localization!.educationFeeAmountValid);
      return false;
    }
    ...
    for (var entry in dynamicFieldControllers.entries) {
      ...
      if (fieldData.text.trim().isEmpty) {
        ToastHelper().showErrorToast(
          localization!.educationFeeDynamicFieldRequired(fieldName),
        );
        return false;
      }
    }
