...
class WaterBillController extends GetxController {
  ...
  Future<void> fetchBillCountries() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint: "${ApiPath.getBillCountriesEndpoint}/water-bill",
      );
  ...
  Future<void> fetchPayBillServices() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint:
            "${ApiPath.getPayBillServicesEndpoint}/${countryController.text}/water-bill",
      );
      if (response.status == Status.completed) {
  ...
  bool validateAmountStep() {
    ...
    if (countryController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.waterBillCountryRequired);
      return false;
    }

    // Validate Service
    if (serviceController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.waterBillServiceRequired);
      return false;
    }

    // Validate Amount
    if (amountText.value.isEmpty && amountController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.waterBillAmountRequired);
      return false;
    }

    final amount = double.tryParse(amountController.text) ?? 0.0;
    if (amount <= 0) {
      ToastHelper().showErrorToast(localization!.waterBillAmountValid);
      return false;
    }
    ...
    for (var entry in dynamicFieldControllers.entries) {
      ...
      if (fieldData.text.trim().isEmpty) {
        ToastHelper().showErrorToast(
          localization!.waterBillDynamicFieldRequired(fieldName),
        );
        return false;
      }
    }
